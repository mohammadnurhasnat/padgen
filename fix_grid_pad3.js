...
const replacement = `  let repeatedNamePattern = null;
  if (gridStyle === 'dots') {
    ...
    backgroundGridStyles.backgroundImage = \`url("data:image/svg+xml;utf8,\${encodeURIComponent(svgStr)}")\`;
    backgroundGridStyles.backgroundSize = '1200px 40px';
  }`;

// Need to escape backticks in the string before passing to replace
code = code.replace(/let repeatedNamePattern = null;[\s\S]*?backgroundGridStyles\.backgroundSize = '1200px 40px';\s*\}/, replacement);
if(code.indexOf('let repeatedNamePattern = null;') === -1) {
  // It means fix_grid_pad2 didn't apply properly or the first one failed.
  code = code.replace(/if \(gridStyle === 'dots'\) \{[\s\S]*?backgroundGridStyles\.backgroundSize = '100% 6\.5mm, 6\.5mm 100%';\s*\}/, replacement);
}
